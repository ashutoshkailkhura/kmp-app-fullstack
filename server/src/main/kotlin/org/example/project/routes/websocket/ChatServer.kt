...
class ChatServer {
    /**
     * A concurrent map associating session IDs to userId.
     */
    val memberNames = ConcurrentHashMap<String, Int>()

    /**
     * Associates a user ID to a set of websockets.
...
//        }
     ...
     */
//    suspend fun memberRenamed(member: ChatSession, to: Int) {
//        // Re-sets the member name.
//        val oldName = memberNames.put(member.sessionId, to) ?: member
...
//    }
    ...
    suspend fun sendTotal(sender: Int, recipient: String) {
        ...
        val res = members.keys()
        val res2 = members.values
        members[recipient]?.send(Frame.Text("sender $sender recipient $recipient"))
    }
...
suspend fun WebSocketSession.sendWebSocketMessage(payload: WebSocketPayload) {
